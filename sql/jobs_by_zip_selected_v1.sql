-- Jobs by ZIP for a specific set of job IDs (v1 dataset names)
-- Uses: st_stage (stage), st_mart (mart); still sources from st_raw_v2 and st_dim_v2
-- Project: kpi-auto-471020

-- 0) Ensure ZIP normalization view exists (safe to re-run)
CREATE OR REPLACE VIEW `kpi-auto-471020.st_stage.locations_zip_norm` AS
WITH src AS (
  SELECT
    id AS locationId,
    LPAD(
...
-- 1) Create a detailed table of those jobs with ZIPs
CREATE OR REPLACE TABLE `kpi-auto-471020.st_stage.selected_jobs_with_zip` AS
...
LEFT JOIN `kpi-auto-471020.st_stage.locations_zip_norm` z
...
-- 2) Create an aggregated table: count of selected jobs by ZIP
CREATE OR REPLACE TABLE `kpi-auto-471020.st_mart.jobs_by_zip_selected` AS
SELECT
  zip_code,
  COUNT(*) AS jobs_count,
  COUNT(DISTINCT jobId) AS jobs_distinct
FROM `kpi-auto-471020.st_stage.selected_jobs_with_zip`
GROUP BY zip_code
